...
else
{
    ...
    if ($action == "changepw") {
      ...
      if ($submit) {
        ...
        if ($validate->validate("match_value", "existingpwd", "existingpasswordincorrect", array($existingpw, $existingpwd)) && $validate->validate("required", "newpw", "ordererrorpassword") && $validate->validate("pwstrength", "newpw", "pwstrengthfail") && $validate->validate("required", "confirmpw", "clientareaerrorpasswordconfirm")) {
      ...
      if ($action == "security") {
        ...
        if ($whmcs->get_req_var("submit")) {
          ...
          if (!$errormessage) {
            ...
            header("Location: clientarea.php?action=changesq&successful=true");
            exit();
          }
...
$ca->setTemplate("clientareaaddfunds");$ca->assign("minimumamount", formatCurrency($addfundsmin));$ca->assign("maximumamount", formatCurrency($addfundsmax));$ca->assign("maximumbalance", formatCurrency($addfundsmaxbal));$ca->assign("amount", format_as_currency($amount));$gatewayslist = showPaymentGatewaysList();$ca->assign("gateways", $gatewayslist);
} else {
  if ($action == "masspay") {
    checkContactPermission("invoices");
