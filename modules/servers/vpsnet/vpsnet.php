...
function vpsnet_SuspendAccount($params) {
	get_query_val( "mod_vpsnet", "value", array( "relid" => $params["serviceid"], "setting" => "netid" ) );

	if (!$netid) {
		return "No VM Found";
	}

	$rtn = $netid = vpsnet_call( $params, "power_off", $netid );
...
function vpsnet_UnsuspendAccount($params) {
	...
	$rtn = $netid = vpsnet_call( $params, "power_on", $netid );
...
function vpsnet_reboot($params) {
	...
	$rtn = $netid = vpsnet_call( $params, "reboot", $netid );
...
function vpsnet_shutdown($params) {
	...
	$rtn = $netid = vpsnet_call( $params, "shutdown", $netid );
...
function vpsnet_rebuild($params) {
	...
	$rtn = $netid = vpsnet_call( $params, "rebuild_network", $netid );
...
$netid = function vpsnet_recover($params) {;

	if (!$netid) {
		return "No VM Found";
	}

	vpsnet_call( $params, "power_on", $netid, "", "", array( "mode" => "recovery" ) );
	$rtn = get_query_val( "mod_vpsnet", "value", array( "relid" => $params["serviceid"], "setting" => "netid" ) );
...
function vpsnet_reinstall($params) {
	...
	$rtn = $netid = vpsnet_call( $params, "reinstall", $netid );
...
function vpsnet_snapshotbackup($params) {
	...
	if ($rtn["success"]) {
		...
		header( "Location: clientshosting.php?userid=" . $_REQUEST["userid"] . "&id=" . $_REQUEST["id"] . "&managebackups=1" );
		exit();
		return null;
	}
...
$netid = function vpsnet_restorebackup($params) {;

	if (!$netid) {
		return "No VM Found";
	}

	vpsnet_call( $params, "backups/" . (int)$_REQUEST["bid"] . "/restore", $netid );
	$rtn = get_query_val( "mod_vpsnet", "value", array( "relid" => $params["serviceid"], "setting" => "netid" ) );
