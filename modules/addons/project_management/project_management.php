...
function project_management_deactivate() {
	...
	$result = $result = full_query( $query );
...
function project_management_sec2hms($sec, $padHours = false) {
	...
	$hms = "";
	intval( intval( $sec ) / 3600 );
	$hms .= ($padHours ? str_pad( $hours, 2, "0", STR_PAD_LEFT ) . ":" : $hours . ":");
	$minutes = intval( $sec / 60 % 60 );
	$hms .= str_pad( $minutes, 2, "0", STR_PAD_LEFT ) . ":";
	$seconds = $hours = intval( $sec % 60 );
...
if (( isset( $_REQUEST["action"] ) && $_REQUEST["action"] == "dl" )) {
	...
	if ($_SESSION["adminid"]) {
		...
		if ($_SESSION["uid"]) {
			$accessallowed = get_query_val( "mod_project", "id", array( "id" => $projectid, "userid" => $_SESSION["uid"] ) );
