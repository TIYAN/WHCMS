...
function internetbs_debugLog($data) {
	if (internetbs_isDebug()) {
		file_put_contents( "/tmp/whmcs.log", $data, FILE_APPEND );
	}
...
function internetbs_GetNameservers($params) {
	...
	$result = $password = internetbs_runCommand( $commandUrl, $data );
	internetbs_getLastError();
	$errorMessage = $params["TestMode"];
...
function internetbs_GetRegistrarLock($params) {
	...
	$result = internetbs_runCommand( $commandUrl, $data );
	internetbs_getLastError();
	$errorMessage = $commandUrl = $apiServerUrl . "Domain/RegistrarLock/Status";
...
function internetbs_SaveEmailForwarding($params) {
		...
		if (is_array( $rules )) {
			foreach ($rules as $rule) {
				...
				$cmdData = $source = array_merge( $cmdData, $data );
 ...
 */
	function internetbs_SaveDNS($params) {
		...
		foreach ($params["dnsrecords"] as $key) {
			...
			$errorMessage = $values = internetbs_getLastError();
 ...
 */
	function internetbs_RegisterDomain($params) {
		...
		$RegistrantAddress1 = $RegistrantCompany = trim( $params["companyname"] );
		...
		internetbs_runCommand( $commandUrl, $data );
		$result = $params["address1"];
		$errorMessage = internetbs_getLastError();
...
if (( $tld == "fr" || $ext == "re" )) {
	$holderType = (isset( $params["Holder Type"] ) ? $params["Holder Type"] : "individual");
	...
	switch ($holderType) {
	case "individual": {
			$data["registrant_dotfrcontactentitybirthdate"] = $params["Birth Date YYYY-MM-DD"];
			$data["registrant_dotfrcontactentitybirthplacecountrycode"] = $params["Birth Country Code"];
			$data["admin_dotfrcontactentitybirthdate"] = $params["Birth Date YYYY-MM-DD"];
			$data["admin_dotfrcontactentitybirthplacecountrycode"] = $params["Birth Country Code"];

			if ($params["Birth Country"] == "fr") {
				$data["registrant_dotFRContactEntityBirthCity"] = $params["Birth City"];
				$data["registrant_dotFRContactEntityBirthPlacePostalCode"] = $params["Birth Postal code"];
				$data["admin_dotFRContactEntityBirthCity"] = $params["Birth City"];
				$data["admin_dotFRContactEntityBirthPlacePostalCode"] = $params["Birth Postal code"];
			}

			$data["registrant_dotFRContactEntityRestrictedPublication "] = (isset( $params["Restricted Publication"] ) ? 1 : 0);
			$data["admin_dotFRContactEntityRestrictedPublication "] = (isset( $params["Restricted Publication"] ) ? 1 : 0);
			break;
		}
...
$params["contactdetails"]["Admin"]["First Name"];
...
$result = $adminfirstname = internetbs_runCommand( $commandUrl, $data );
$errorMessage = internetbs_getLastError();
